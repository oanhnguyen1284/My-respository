...
namespace FoursquareAngularJS.Web.Controllers
{
    public class PlacesController : BaseApiController
    {
        ...
        {
            try {
                var results = TheRepository.SavePlace(bookmarkedPlace);
                if (results == 1)
                {
                ...
                {
                    return Request.CreateResponse(HttpStatusCode.NotModified, bookmarkedPlace.VenueName, bookmarkedPlace.UserName);
                }
                else {
